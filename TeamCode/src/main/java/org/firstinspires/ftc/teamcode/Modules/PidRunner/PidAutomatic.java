...
public class PidAutomatic implements IRobotModule {
    ...
    public void Update() {
        ...
        if (Configs.GeneralSettings.IsAutonomEnable) {
            ...
            _driverTrain.Drive(
                    Math.abs(_PIDFTurn.Err) < Configs.Automatic.TurnSensitivity ? _PIDFForward.Update(dif.Abs()) / Battery.ChargeDelta : 0,
                    _PIDFTurn.Update(Gyroscope.ChopAngle(_gyro.GetRadians() - Math.atan2(dif.Y, dif.X))) / Battery.ChargeDelta);
        }
    }
